 ...
 * 成员列表适配器
 */
public class MembersAdapter extends ArrayAdapter<MemberInfo.DataEntity> {
    private static final String TAG = MembersAdapter.class.getSimpleName();
    private LiveView mLiveView;
    private MembersDialog membersDialog;

    public MembersAdapter(Context context, int resource, ArrayList<MemberInfo.DataEntity> objects, LiveView liveView, MembersDialog dialog) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        final MemberInfo.DataEntity data = getItem(position);
        final String selectId = data.getUserId();
        holder.id.setText(data.getUserName());
