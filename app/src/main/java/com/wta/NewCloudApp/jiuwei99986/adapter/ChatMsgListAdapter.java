...
import com.wta.NewCloudApp.jiuwei99986.widget.CustomTextView;
...
public class ChatMsgListAdapter extends BaseAdapter implements AbsListView.OnScrollListener {
     ...
     * 通过名称计算颜色
     */
    private int calcNameColor(String strName, int type, int sign) {
        ...
        if (type == 1) {
            ...
            switch (idx & 0x7) {
                case 1:
                    return context.getResources().getColor(R.color.appColor);
                case 2:
                    return context.getResources().getColor(R.color.appColor);
                case 3:
                    return context.getResources().getColor(R.color.appColor);
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (item.getType() == -1) {
            ...
            // 根据名称计算颜色
           // holder.textItem.getBackground().setAlpha(255);
            //bug

            spanString.setSpan(new ForegroundColorSpan(calcNameColor(item.getSenderName(), 0,0)),
                    0, item.getSenderName().length(), Spannable.SPAN_EXCLUSIVE_INCLUSIVE);
            spanString.setSpan(new ForegroundColorSpan(calcNameColor(item.getContext(), 1,item.getSign())),
                    item.getSenderName().length(), item.getContext().length()+item.getSenderName().length()+2, Spannable.SPAN_EXCLUSIVE_INCLUSIVE);
